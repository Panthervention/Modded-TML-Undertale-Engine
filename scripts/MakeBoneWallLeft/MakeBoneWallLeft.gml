...
function MakeBoneWallLeft() {
	...
	{
	    ii += 1
	    b[ii] = MakeBoneH((((battle_board.x - battle_board.left) - (LENGTH / 2)) -6), ((battle_board.y - battle_board.up) + i), LENGTH - 6, 0, 0, COLOR, 0, 0, 0, ((DURATION + (PAUSE * 2)) + 10 + ANIM))
	    with (b[ii])
	    {
	        Anim_Create(id, "x", 0, 0, x, LENGTH, ANIM, PAUSE)
	        Anim_Create(id, "x", 0, 0, (x + LENGTH), ((-LENGTH) * 2), ANIM, ((ANIM + PAUSE) + DURATION))
