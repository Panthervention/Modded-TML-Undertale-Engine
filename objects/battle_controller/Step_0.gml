//This is hotkeys for debug
//Debug by FDY
if Player_GetHp() + Player_GetKR() >= Player_GetHpMax()
	Player_SetKR(Player_GetHpMax() - Player_GetHp())

if (keyboard_check(ord("B")) && keyboard_check(ord("U")) && keyboard_check_pressed(ord("G")))
{
	Anim_Destroy(id,"alpha")
    if (global.debug == false)
		Anim_Create(id,"alpha",0,0,0,1,30)
    else if (global.debug == true)
	{
		room_speed = 60
		Anim_Create(id,"alpha",0,0,1,-1,30)
	}
}
if alpha <= 0 global.debug = false
...
if(_state==BATTLE_STATE.MENU){
	...
	if(_menu==BATTLE_MENU.FIGHT_TARGET){
		...
		if(Input_IsPressed(INPUT.UP)){
			...
			if(enemy>=0){
				...
				Anim_Create(battle_soul,"x",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.x,(-battle_soul.x)+(battle_board.x-battle_board.left-5+40),5);
				Anim_Create(battle_soul,"y",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.y,(-battle_soul.y)+(battle_board.y-battle_board.up-5+36+32*_menu_choice_enemy),5);
			}
	...
	if(_menu==BATTLE_MENU.ACT_TARGET){
		...
		if(Input_IsPressed(INPUT.CANCEL)){
			Battle_SetMenu(BATTLE_MENU.BUTTON);
			Anim_Create(battle_soul,"x",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.x,(-battle_soul.x)+193,10);
			Anim_Create(battle_soul,"y",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.y,(-battle_soul.y)+453,10);
		}
		//确定
		if(Input_IsPressed(INPUT.CONFIRM)){
			audio_play_sound(snd_menu_confirm,0,false);
	...
	if(_menu==BATTLE_MENU.ACT_ACTION){
		...
		if(Input_IsPressed(INPUT.UP)){
			...
			if(action>=0){
				...
				Anim_Create(battle_soul,"x",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.x,(-battle_soul.x)+(battle_board.x-battle_board.left-5+40+256*(_menu_choice_action%2)),5);
				Anim_Create(battle_soul,"y",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.y,(-battle_soul.y)+(battle_board.y-battle_board.up-5+36+32*floor(_menu_choice_action/2)),5);			
			}
		}else if(Input_IsPressed(INPUT.DOWN)){
			var action=_menu_choice_action+2;
...
if(_state!=BATTLE_STATE.RESULT && Battle_GetEnemyNumber()==0){
	...
	text+=Lang_GetString("battle_controller.result.won");
	Player_SetExp(Player_GetExp()+Battle_GetRewardExp());
	Player_SetGold(Player_GetGold()+Battle_GetRewardGold());
	if(Player_UpdateLv()){
		text+="&"+Lang_GetString("battle_controller.result.lv_up");
